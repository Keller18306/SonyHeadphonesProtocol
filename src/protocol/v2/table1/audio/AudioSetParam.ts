...
export class AudioSetParam extends AbstractCommand {
    public dataType: FrameDataType = FrameDataType.DATA_MDR;
    public command: CommandTable1 = CommandTable1.AUDIO_SET_PARAM;

    public static fromBuffer(buffer: Buffer): AudioSetParam {
        const reader = new ReadBuffer(buffer);

        if (reader.readUInt8() !== CommandTable1.AUDIO_SET_PARAM) {
        ...
        switch (inquiredType) {
            ...
            case AudioInquiredType.CONNECTION_MODE_WITH_LDAC_STATUS:
                return new AudioSetParam(AudioParamConnectionMode.fromBuffer(inquiredType, reader));
            case AudioInquiredType.UPSCALING:
                return new AudioSetParam(AudioParamUpscaling.fromBuffer(reader));
            case AudioInquiredType.BGM_MODE:
                return new AudioSetParam(AudioParamBgm.fromBuffer(reader));
        }
