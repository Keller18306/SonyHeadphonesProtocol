...
export class AudioNtfyParam extends AbstractCommand {
    public dataType: FrameDataType = FrameDataType.DATA_MDR;
    public command: CommandTable1 = CommandTable1.AUDIO_NTFY_PARAM;

    public static fromBuffer(buffer: Buffer): AudioNtfyParam {
        const reader = new ReadBuffer(buffer);

        if (reader.readUInt8() !== CommandTable1.AUDIO_NTFY_PARAM) {
        ...
        switch (inquiredType) {
            ...
            case AudioInquiredType.CONNECTION_MODE_WITH_LDAC_STATUS:
                return new AudioNtfyParam(AudioParamConnectionMode.fromBuffer(inquiredType, reader));
            case AudioInquiredType.UPSCALING:
                return new AudioNtfyParam(AudioParamUpscaling.fromBuffer(reader));
            case AudioInquiredType.BGM_MODE:
                return new AudioNtfyParam(AudioParamBgm.fromBuffer(reader));
        }
