...
export class PlayRetStatus extends AbstractCommand {
    public dataType: FrameDataType = FrameDataType.DATA_MDR;
    public command: CommandTable1 = CommandTable1.PLAY_RET_STATUS;

    public static fromBuffer(buffer: Buffer): PlayRetStatus {
        const reader = new ReadBuffer(buffer);

        if (reader.readUInt8() !== CommandTable1.PLAY_RET_STATUS) {
        ...
        return new PlayRetStatus(inquiredType,
