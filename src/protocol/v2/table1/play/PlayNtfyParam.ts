...
export class PlayNtfyParam extends AbstractCommand {
    public dataType: FrameDataType = FrameDataType.DATA_MDR;
    public command: CommandTable1 = CommandTable1.PLAY_NTFY_PARAM;

    public static fromBuffer(buffer: Buffer): PlayNtfyParam {
        const reader = new ReadBuffer(buffer);

        if (reader.readUInt8() !== CommandTable1.PLAY_NTFY_PARAM) {
        ...
        switch (inquiredType) {
            case PlayInquiredType.PLAYBACK_CONTROL_WITH_CALL_VOLUME_ADJUSTMENT:
                return new PlayNtfyParam(PlayParamPlayback.fromBuffer(reader));
            case PlayInquiredType.MUSIC_VOLUME:
            case PlayInquiredType.CALL_VOLUME:
                return new PlayNtfyParam(PlayParamVolume.fromBuffer(inquiredType, reader));
            case PlayInquiredType.MUSIC_VOLUME_WITH_MUTE:
            case PlayInquiredType.CALL_VOLUME_WITH_MUTE:
                return new PlayNtfyParam(PlayParamVolumeWithMute.fromBuffer(inquiredType, reader));
        }
