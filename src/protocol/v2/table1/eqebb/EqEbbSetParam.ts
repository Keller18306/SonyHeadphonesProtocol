...
export class EqEbbSetParam extends AbstractCommand {
    public dataType: FrameDataType = FrameDataType.DATA_MDR;
    public command: CommandTable1 = CommandTable1.EQEBB_SET_PARAM;

    public static fromBuffer(buffer: Buffer): EqEbbSetParam {
        const reader = new ReadBuffer(buffer);

        if (reader.readUInt8() !== CommandTable1.EQEBB_SET_PARAM) {
        ...
        switch (inquiredType) {
            case EqEbbInquiredType.EBB:
                return new EqEbbSetParam(EqEbbParamEbb.fromBuffer(reader));
            case EqEbbInquiredType.PRESET_EQ:
            case EqEbbInquiredType.PRESET_EQ_NONCUSTOMIZABLE:
                return new EqEbbSetParam(EqEbbParamPresetEq.fromBuffer(inquiredType, reader));
            case EqEbbInquiredType.PRESET_EQ_AND_ULT_MODE:
                return new EqEbbSetParam(EqEbbParamPresetEqAndUlt.fromBuffer(reader));
        }
