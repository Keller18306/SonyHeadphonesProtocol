...
export class PowerRetParam extends AbstractCommand {
    public dataType: FrameDataType = FrameDataType.DATA_MDR;
    public command: CommandTable1 = CommandTable1.POWER_RET_PARAM;

    public static fromBuffer(buffer: Buffer): PowerRetParam {
        const reader = new ReadBuffer(buffer);

        if (reader.readUInt8() !== CommandTable1.POWER_RET_PARAM) {
        ...
        switch (inquiredType) {
            case PowerInquiredType.AUTO_POWER_OFF:
                return new PowerRetParam(PowerParamAutoPowerOff.fromBuffer(reader));
            case PowerInquiredType.AUTO_POWER_OFF_WEARING_DETECTION:
                return new PowerRetParam(PowerParamAutoPowerOffWearingDetection.fromBuffer(reader));
            case PowerInquiredType.POWER_SAVE_MODE:
            case PowerInquiredType.CARING_CHARGE:
            case PowerInquiredType.BT_STANDBY:
                return new PowerRetParam(PowerParamEnabled.fromBuffer(inquiredType, reader));
            case PowerInquiredType.BATTERY_SAFE_MODE:
                return new PowerRetParam(PowerParamBatterySafeMode.fromBuffer(reader));
        }
