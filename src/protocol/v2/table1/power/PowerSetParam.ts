...
export class PowerSetParam extends AbstractCommand {
    public dataType: FrameDataType = FrameDataType.DATA_MDR;
    public command: CommandTable1 = CommandTable1.POWER_SET_PARAM;

    public static fromBuffer(buffer: Buffer): PowerSetParam {
        const reader = new ReadBuffer(buffer);

        if (reader.readUInt8() !== CommandTable1.POWER_SET_PARAM) {
        ...
        switch (inquiredType) {
            case PowerInquiredType.AUTO_POWER_OFF:
                return new PowerSetParam(PowerParamAutoPowerOff.fromBuffer(reader));
            case PowerInquiredType.AUTO_POWER_OFF_WEARING_DETECTION:
                return new PowerSetParam(PowerParamAutoPowerOffWearingDetection.fromBuffer(reader));
            case PowerInquiredType.POWER_SAVE_MODE:
            case PowerInquiredType.CARING_CHARGE:
            case PowerInquiredType.BT_STANDBY:
                return new PowerSetParam(PowerParamEnabled.fromBuffer(inquiredType, reader));
            case PowerInquiredType.BATTERY_SAFE_MODE:
                return new PowerSetParam(PowerParamBatterySafeMode.fromBuffer(reader));
        }
