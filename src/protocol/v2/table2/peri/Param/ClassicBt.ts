...
interface PeriDevice {
    address: string,
    status: number, // скорее всего 0 - отключено, 1 - подключено, 2 - закреплено
    deviceName: string
}

export class PeriParamClassicBt extends BasicCommand {
    public type: PeripheralInquiredType = PeripheralInquiredType.PAIRING_DEVICE_MANAGEMENT_CLASSIC_BT;

    public static fromBuffer(reader: ReadBuffer): PeriParamClassicBt {
        ...
        return new PeriParamClassicBt(devices, playbackrightDeviceIndex);
    }
